...
#define NODE_ID          2
#define SEND_INTERVAL    500  // ← diperkecil dari 500ms ke 100ms
...
bool timerPulseLatchA = false;

bool latchedStartB = false;
bool lastTimerB = false;
bool timerPulseLatchB = false;
...
void loop() {
  ...
  if (curEmergA) {
    latchedStartA = false;
    timerPulseLatchA = false;
  } else {
    if (curStartA) latchedStartA = true;
    if (!curTimerA && lastTimerA) {  // falling edge = motor berhenti
      latchedStartA = false;
      timerPulseLatchA = true;       // simpan pulse sampai terkirim
    }
  }
  ...
  if (curEmergB) {
    latchedStartB = false;
    timerPulseLatchB = false;
  } else {
    if (curStartB) latchedStartB = true;
    if (!curTimerB && lastTimerB) {  // falling edge = motor berhenti
      latchedStartB = false;
      timerPulseLatchB = true;       // simpan pulse sampai terkirim
    }
  }
  ...
  if (millis() - lastSendTime >= SEND_INTERVAL) {
    ...
    packetData.headA_timer     = timerPulseLatchA;
    ...
    packetData.headB_timer     = timerPulseLatchB;
    ...
    if (result == ESP_OK) {
      timerPulseLatchA = false;  // reset setelah berhasil terkirim
      timerPulseLatchB = false;
      Serial.printf("Sent ID:2 | A:[%d-%d-%d] B:[%d-%d-%d]\n",
                    latchedStartA, packetData.headA_timer, curEmergA,
                    latchedStartB, packetData.headB_timer, curEmergB);
    } else {
      Serial.println("Send Failed!");
      // timerPulseLatch TIDAK direset → akan dicoba kirim lagi di interval berikutnya
    }
